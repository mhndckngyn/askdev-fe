...
import { Box, IconButton, Typography, Popover, Chip } from '@mui/material';
import { useMantineColorScheme } from '@mantine/core';
import ThumbUpIcon from '@mui/icons-material/ThumbUp';
import ThumbDownIcon from '@mui/icons-material/ThumbDown';
import CommentIcon from '@mui/icons-material/Comment';
import FavoriteIcon from '@mui/icons-material/Favorite';
import { voteQuestion, getVoteStatus, getQuestion } from './QuestionServices';
...
export default function InteractionBar({ question }: Props) {
  ...
  useEffect(() => {
    ...
    const fetchQuestion = async () => {
      try {
        const response: ApiResponse = await getQuestion(question.id);
        if (response.success) {
          setData(response.content);
        }
        const voteStatusResponse: ApiResponse = await getVoteStatus(
          question.id,
        );
  ...
  const handleLike = async () => {
    if (!question.id || question.id.trim() === '') return;

    try {
      setAnimateVote(true);
      await voteQuestion(question.id, 1);
      const response: ApiResponse = await getQuestion(question.id);
      if (response.success) {
        setData(response.content);
      }
  ...
  return (
    <Box
      ...
      <Box
        ...
        {/* Comment Section */}
        <Box
          component={data ? Link : 'div'}
          to={data ? `/questions/${data.id}` : undefined}
          sx={{
            display: 'flex',
            alignItems: 'center',
            gap: 1,
