...
export default function ImageGrid({ files }: ImageGridProps) {
  ...
  const documentFiles = files.filter((file) =>
    /\.(pdf|docx?|xlsx?|pptx?)$/i.test(file),
  );
  ...
  const renderImages = () => {
    switch (imageFiles.length) {
      ...
      case 2:
        return (
          <Box
            sx={{ display: 'grid', gap: 0.5, gridTemplateColumns: '1fr 1fr' }}>
      ...
      default: // 5 ảnh trở lên
        return (
          <Box
            sx={{ display: 'grid', gap: 0.5, gridTemplateRows: 'auto auto' }}>
            <Box
              sx={{
                display: 'grid',
                gridTemplateColumns: '1fr 1fr',
            ...
            <Box
              ...
              {imageFiles.slice(2, 4).map((img, index) => (
                <Box
                  key={index + 2}
                  sx={{
                    height: '200px', // Same height as the first row
                    '&:hover': {
                      border: '1px solid var(--button-color)',
                    },
                    position: 'relative',
